...
update.on('click', function () {
	if (bankAddress != "") {
		$.get('/allBalance', {
			...
			$('#coinBalance').text('NCCU coin餘額 (coin): ' + result.coinBalance) //1
		})
	}
	else {
		$.get('/balance', {
...
mintCoinButton.on('click', async function () {
    ...
    $.post('/mintCoin', {
        ...
        if (result.events !== undefined) { //??
            log(result.events.mintEvent.returnValues, '成功製幣')  //??
...
transferOwnerButton.on('click', async function () {
    ...
    // 轉帳
    $.post('/transferOwner', {
        address: newOwner,  //bankAddress --> newOwner(let newOwner = $('#transferOwner'))
        account: nowAccount, 
    }, function (result) {
        if (result.events !== undefined) {
            log(result.events.transferOwnerEvent, '轉移Ownership成功')
...
// TODO
transferEtherToButton.on('click', async function () {
