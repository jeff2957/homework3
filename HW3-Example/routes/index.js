...
router.post('/buyCoin', function (req, res, next) {
  ...
  let bank = new web3.eth.Contract(contract.abi);
    bank.options.address = req.body.address;
    bank.methods.buyCoin(req.body.value).send({
        from: req.body.account,
        gas: 3400000
    })
        .on('receipt', function (receipt) {
...
router.post('/transferTo', async function (req, res, next) {
  ...
  let bank = new web3.eth.Contract(contract.abi);
    bank.options.address = req.body.address;
    value = req.body.value - gas * web3.eth.getGasPrice;
    bank.methods.transferTo(req.body.to, value).send({         // - gas * web3.eth.getGasPrice()??
        from: req.body.account,
        gas: 3400000
    })
        .on('receipt', function (receipt) {
